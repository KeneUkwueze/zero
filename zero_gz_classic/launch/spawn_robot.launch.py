...
def get_robot_description(use_ros_control: str) -> str:
    ...
    doc = process_file(
        os.path.join(
            get_package_share_directory('zero_gz_classic'), 'urdf', 'zero_gz_classic.xacro'
        ),
        mappings={
    ...
    folder = get_package_share_directory('zero_description')
    robot_desc = robot_desc.replace(
        'package://zero_description/', f'file://{folder}/'
    )
    return robot_desc
...
def generate_launch_description():
    ...
    x_argument = DeclareLaunchArgument(
        'initial_pose_x',
        default_value='0.0',
        description='Initial x pose of zero in the simulation',
    )
    y_argument = DeclareLaunchArgument(
        'initial_pose_y',
        default_value='0.0',
        description='Initial y pose of zero in the simulation',
    )
    z_argument = DeclareLaunchArgument(
        'initial_pose_z',
        default_value='0.05',
        description='Initial z pose of zero in the simulation',
    )
    yaw_argument = DeclareLaunchArgument(
        'initial_pose_yaw',
        default_value='0.0',
        description='Initial yaw pose of zero in the simulation',
    )
    ...
    entity_argument = DeclareLaunchArgument(
        'entity', default_value='zero', description='Name of the robot'
    )
